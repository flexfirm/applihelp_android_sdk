...
<h3 name="initialize">初期化（アクティベーション）</h3>
このメソッド内でアプリヘルプサーバへの初期登録処理が実行されます。  
AppHelpの一部のメソッドを呼出す前に実行されている必要がありますが、**不要なアプリヘルプサーバへのアクセスを減らすため、必ず各該当メソッドを呼出す直前に実行してください。**（複数回呼出された場合、サーバへのアクセスは内部でスキップされます。）  
該当メソッドについては各メソッドの説明を参照してください。  

なお、このメソッドは
`android.intent.category.LAUNCHER`カテゴリのActivityクラスにて実行してください。  

PUSH通知受信(GCM)を利用する場合
```java
appHelp.install(MainActivity.this, "your.application.package", "Your-Sender-ID");
```
PUSH通知受信(GCM)利用しない場合
```java
...
**※このメソッドは__[初期化（アクティベーション）](#Usage)__を実行しておく必要があります。**
```java
//初期化（アクティベーション）
appHelp.install(MainActivity.this, "your.application.package", "123456789123");//PUSH通知受信(GCM)を利用の場合
//appHelp.install(MainActivity.this, "your.application.package", null); //PUSH通知受信(GCM)なしの場合
...
**※「Applihelp　メイン画面表示」と同様、このメソッドは__[初期化（アクティベーション）](#Usage)__を実行しておく必要があります。**
...
**※このメソッドを利用するには、1度でも__[初期化（アクティベーション）](#Usage)__が行われている必要があります。**
